    ...
    var methods = {
            ...
            handleFileSelect = function(evt) {
                ...
                for (var i = 0, f; f = files[i]; i++) {
                    ...
                    if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
                        methods.showMessage.apply(this,["imgErrMsg","无效的文件类型"]);
                        continue;
                    }
        ...
        tableWidget: function(mode){
            ...
            var tblCntr = $('<div/>',{ //Outer Container Div
                ...
                ).append($('<label/>',{	for:"tblBorder"+idExtn, text:"边框尺寸"}
                )).append($('<input/>',{
                    id:"tblBorder"+idExtn,
                    type:"text",
                    class:"form-control form-control-width",
                    value:1
                })).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"间距"}
                )).append($('<input/>',{
                    id:"tblCellspacing"+idExtn,
                    type:"text",
                    class:"form-control form-control-width",
                    value:1
                })).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"边距"}
                )).append($('<input/>',{
        ...
        init : function( options )
        {
            var fonts = { "微软雅黑"      :"",
                ...
                "加拉蒙字体"	 : "garamond,serif",
                "Georgia"	 	 : "georgia,serif",
                "大河马字体" 		 : "tahoma,sans-serif",
            ...
            var menuItems = { 'fonteffects': true,
                ...
                'insert_link': { "modal": true,
                    ...
                    "onSave":function(){
                        ...
                        if(targetURL ==''){
                            methods.showMessage.apply(editorObj,["errMsg","请输入链接"]);
                            return false;
                        }
                        if(!targetURL.match(urlPattern)){
                            methods.showMessage.apply(editorObj,["errMsg","请输入正确的链接"]);
                            return false;
                        }
                        if(range=='' && targetText==''){
                ...
                'insert_img'	: { "modal": true,
                    ...
                    "tooltip": "插入图片",       /*Insert Image*/
                    "modalHeader": "插入图片",   /*Insert Image*/
                    ...
                    "onSave": function(){
                        ...
                        if($('#imageList').data('current')){
                            ...
                            methods.showMessage.apply(this,["imgErrMsg","请选择一张图片"]);
                            return false;
                        }
